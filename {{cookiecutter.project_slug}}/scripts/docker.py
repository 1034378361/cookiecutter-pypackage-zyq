...
def run_docker_container(tag=None, ports=None, volumes=None, env_vars=None,
                        command=None, interactive=False, remove=True):
    """运行Docker容器。"""
    project_name = get_project_name()
...
def parse_args():
    ...
    build_parser.add_argument("--build-arg", action="append", help="构建参数 (格式: KEY=VALUE)")

    # 运行命令
    run_parser = subparsers.add_parser("run", help="运行Docker容器")
    run_parser.add_argument("-t", "--tag", help="Docker镜像标签")
    run_parser.add_argument("-p", "--port", action="append", help="端口映射 (格式: HOST:CONTAINER)")
    run_parser.add_argument("-v", "--volume", action="append", help="卷映射 (格式: HOST:CONTAINER)")
    run_parser.add_argument("-e", "--env", action="append", help="环境变量 (格式: KEY=VALUE)")
