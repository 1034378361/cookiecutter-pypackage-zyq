...
def configure_pip_mirror():
    ...
    # 写入pip配置
    pip_conf_file = pip_conf_dir / ("pip.ini" if platform.system() == "Windows" else "pip.conf")
...
def install_pdm():
    ...
    if not is_command_available("pdm"):
        log("安装PDM...")
        run_command([sys.executable, "-m", "pip", "install", "--no-cache-dir", "pdm"], shell=False)
        log("PDM安装完成")
    else:
        log("PDM已安装，跳过。")
...
def install_pre_commit():
    ...
    if not is_command_available("pre-commit"):
        log("安装pre-commit...")
        run_command([sys.executable, "-m", "pip", "install", "--no-cache-dir", "pre-commit"], shell=False)
        log("pre-commit安装完成")
    else:
...
def install_dependencies():
    ...
    if Path("pyproject.toml").exists():
        ...
        run_command([sys.executable, "-m", "pip", "install", "--no-cache-dir", "-r", "requirements.txt"], shell=False)

        # 检查是否有开发依赖
        if Path("requirements-dev.txt").exists():
            log("安装开发环境依赖...")
            run_command([sys.executable, "-m", "pip", "install", "--no-cache-dir", "-r", "requirements-dev.txt"], shell=False)
